...
function validate(value: CreateSpaceInput, intent: "draft"|"pending", files: File[], verification?:VerificationInput) {
  if (!value.name?.trim()) return "공간명을 입력해 주세요.";
  ...
  if (!value.spaceType || !value.shortDescription || !value.description || !value.mainRegion || !value.detailedRegion || !value.address) return "필수 공간 정보를 모두 입력해 주세요.";
  ...
  if (!value.usageRules?.trim()) return "이용 규칙을 입력해 주세요.";
