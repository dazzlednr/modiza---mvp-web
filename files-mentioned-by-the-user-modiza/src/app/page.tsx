...
export default async function Home() {const communities=await listCommunities(createServerSupabaseClient());
  return <>
    <section className="hero">
      <div className="container hero-grid">
        <div>
          <p className="eyebrow">Meet your people in Daegu</p>
          <h1 className="hero-title">취향이 맞는 사람들과<br /><span>우리만의 모임</span>을 시작해요.</h1>
    ...
    <section className="section">
      <div className="container"><p className="eyebrow">Find your interest</p><h2 className="section-title">어떤 취향을 나누고 싶나요?</h2><CommunityExplorer items={communities} compact /></div>
    </section>

    <section className="section" style={{ background: "var(--background-secondary)" }}>
      <div className="container"><p className="eyebrow">Recruiting now</p><h2 className="section-title">지금 모집 중인 커뮤니티</h2><p className="muted">오늘부터 가볍게 시작할 수 있는 모임이에요.</p><CommunityGrid items={communities.slice(0, 4)} /></div>
    </section>
    ...
    <section className="section" style={{ background: "var(--background-secondary)" }}>
      <div className="container"><p className="eyebrow">New communities</p><h2 className="section-title">새롭게 시작한 커뮤니티</h2><CommunityGrid items={communities.slice(8, 12)} /></div>
    </section>

    <section className="section">
      <div className="container"><h2 className="section-title">이번 주, 참여할 수 있는 모임</h2><p className="muted">날짜가 가까운 모임부터 가볍게 참여해보세요.</p><div className="grid cards">{communities.slice(2, 6).map((c) => <Link className="panel" href={`/communities/${c.slug}`} key={c.id}><span className="tag"><CalendarDays size={14} />{c.nextMeetingAt?new Date(c.nextMeetingAt).toLocaleDateString("ko-KR",{timeZone:"Asia/Seoul"}):"일정 준비 중"}</span><h3>{c.name}</h3><p className="muted">{c.shortDescription}</p><div className="meta"><MapPin size={14} />{c.mainRegion} · {Math.max(0,c.capacity-c.currentMembers)}자리 남음</div></Link>)}</div></div>
    </section>
