...
export function CommunityForm({ spaces, community, suggested, assisted = false }: { spaces: SpaceOption[]; community?: Community; suggested?: Partial<CommunityFormValues>; assisted?: boolean }) {
  ...
  const basicComplete = Boolean(values.name.trim() && values.shortDescription.trim() && values.description.trim() && values.targetAudience?.trim() && values.capacity > 0);
  const activityComplete = Boolean(values.activityDescription.trim() && values.expectedDurationHours >= .5);
  ...
  return <form className="form community-wizard" onSubmit={submit} noValidate>
    ...
    {step === 1 && <section className="panel wizard-panel"><p className="eyebrow">Step 1</p><h2>기본 정보</h2><div className="grid form-grid"><label>커뮤니티명<input className="field" value={values.name} maxLength={80} placeholder="예: 퇴근 후 독서모임" onChange={(event) => update("name", event.target.value)} /><FieldTip>활동과 분위기를 함께 담으면 모임의 특징을 쉽게 전달할 수 있어요.</FieldTip></label><label>카테고리<select className="field" value={values.category} onChange={(event) => update("category", event.target.value as CommunityFormValues["category"])}>{communityCategories.map((category) => <option key={category}>{category}</option>)}</select></label><label>참여 대상<input className="field" value={values.targetAudience ?? ""} placeholder="예: 대구에 거주하는 20~30대" onChange={(event) => update("targetAudience", event.target.value)} /></label><label>최대 인원<input className="field" type="number" min={1} value={values.capacity} onChange={(event) => update("capacity", Number(event.target.value))} /></label></div><label>한 줄 소개<input className="field" maxLength={160} value={values.shortDescription} placeholder="예: 책을 매개로 사람들과 편하게 이야기하는 독서모임입니다." onChange={(event) => update("shortDescription", event.target.value)} /><FieldTip>한 문장만으로 어떤 모임인지 쉽게 이해할 수 있도록 작성해보세요.</FieldTip></label><label>커뮤니티 소개<textarea className="field" rows={6} value={values.description} placeholder="어떤 모임인지, 어떻게 진행되는지, 어떤 분들과 함께하고 싶은지 소개해주세요." onChange={(event) => update("description", event.target.value)} /><FieldTip>어떤 모임인지, 어떻게 진행되는지, 어떤 분들이 참여하면 좋은지 차례로 소개해보세요.</FieldTip></label></section>}

    {step === 2 && <section className="panel wizard-panel">
      <p className="eyebrow">Step 2</p><h2>활동 정보</h2>
      <label>어떤 활동을 진행하나요?<textarea className="field" rows={5} value={values.activityDescription} placeholder="예: 매회 한 권을 정해 읽고, 인상 깊었던 부분과 서로의 생각을 편하게 나눕니다." onChange={(event) => update("activityDescription", event.target.value)} /><FieldTip>참여자가 실제 모임 모습을 그릴 수 있도록 진행 순서와 활동 방식을 알려주세요.</FieldTip></label>
    ...
    {step === 3 && <section className="panel wizard-panel"><p className="eyebrow">Step 3</p><h2>일정 및 위치</h2><label>모임 시작 일시<input className="field" type="datetime-local" value={values.nextMeetingAt} min={localDateTime(new Date(Date.now() + 60 * 60 * 1000).toISOString())} onChange={(event) => update("nextMeetingAt", event.target.value)} /></label><RegionFields mainRegion={values.mainRegion} detailedRegion={values.detailedRegion} customRegion={values.customRegion} onChange={(region) => setValues((current) => ({ ...current, ...region }))} /><div className="grid form-grid"><label>희망 최소 예산<input className="field" type="number" min={0} value={values.budgetMin} onChange={(event) => update("budgetMin", Number(event.target.value))} /></label><label>희망 최대 예산<input className="field" type="number" min={0} value={values.budgetMax} onChange={(event) => update("budgetMax", Number(event.target.value))} /></label><label>이동 가능 범위<select className="field" value={values.travelRange} onChange={(event) => update("travelRange", event.target.value)}><option>선택 지역 중심</option><option>인접 구까지</option><option>대구 전역</option></select></label><label>참가비<input className="field" type="number" min={0} value={values.participationFee} onChange={(event) => update("participationFee", Number(event.target.value))} /></label></div><div className="automatic-window"><strong>커뮤니티를 등록하면 바로 모집이 시작됩니다.</strong><span>참여자가 일정 직전까지 신청할 수 있도록 모임 시작 1시간 전에 자동으로 모집이 마감됩니다.</span></div></section>}
