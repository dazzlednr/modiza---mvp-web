...
import type { Community, CommunityFormValues, CommunityStatus, RecruitmentStatus } from "@/types/community";

const ownerSelect = "*,community_applications(count)";

function map(row: any): Community {
  return {
    ...
    category: row.category as Community["category"], customCategory: row.custom_category, shortDescription: row.short_description,
...
const payload = (values: Partial<CommunityFormValues>) => ({
  ...
  ...(values.customCategory !== undefined && { custom_category: values.category === "기타" ? values.customCategory || null : null }),
...
export async function changeCommunityStatus(db: SupabaseClient, id: string, status: CommunityStatus) {
  ...
  if (status === "published") {
    if (!community.name || !community.category || !community.shortDescription || !community.description || !community.mainRegion || !community.detailedRegion || !community.nextMeetingAt || community.capacity < 1 || !community.thumbnailStoragePath) throw new Error("PUBLISH_VALIDATION_FAILED");
    if (community.linkedSpaceId) {
      const { getSpaceById } = await import("@/repositories/spaceRepository");
      const space = await getSpaceById(db, community.linkedSpaceId);
      if (!space || space.status !== "active") throw new Error("SPACE_NOT_AVAILABLE");
